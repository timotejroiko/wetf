...
```html
<script src="https://unpkg.com/wetf/umd/wetf.min.js"></script>
...
If an Asynchronous compression method is set in the packer options, `pack` will instead return a Promise that resolves into Uint8Array.
...
| compression | string \| function | - | Enable data compression and specify the compression method \* |

\* **Wetf** has two built-in compression methods and also allows a user-defined compression function.  
If compression is set to `zlib`, it will use NodeJS's native zlib module (only available in NodeJS).  
If compression is set to `compressionstream`, it will use the browser's native CompressionStream object. This method is Asynchronous and will make `pack` return a Promise (available in modern browsers and NodeJS 18+).  
If compression is set to a user-defined function, the packer will call that function during packing. The function can be Asynchronous, and its signature should be `(Uint8Array) => Uint8Array | Promise<Uint8Array>`.
...
Compressed data is automatically decompressed by the method set in the options or by one of the built-in decompression methods.  
If an Asynchronous decompression method is set in the unpacker options, `unpack` will instead return a Promise that resolves into the JavaScript value.
...
| option | type | default | description |
|---|---|---|---|
| atomTable | object | [Default Atom Table](#default-atom-table) | Defines a table of atoms and the fixed values they represent. \* When an atom is decoded, it will be replaced by the corresponding value, or converted to a string if no fixed value is found |
| decoding | [Decoding Options](#decoding-options) | {} | Customize decoding behavior |
| decompression | string \| function | - | Specify the decompression method \* |

\* **Wetf** has two built-in decompression methods and also allows a user-defined decompression function.  
If decompression is set to `zlib`, it will use NodeJS's native zlib module (only available in NodeJS).  
If decompression is set to `decompressionstream`, it will use the browser's native DecompressionStream object. This method is Asynchronous and will make `unpack` return a Promise (available in modern browsers and NodeJS 18+).  
If decompression is set to a user-defined function, the unpacker will call that function during unpacking. The function can be Asynchronous, and its signature should be `(Uint8Array | Buffer) => Uint8Array | Buffer | Promise<Uint8Array> | Promise<Buffer>`.

#### Default Atom Table

A table of atoms and the JavaScript values they represent.
...
When a custom atom table is defined, it will replace the entire default table. To simply add an atom to the table, the user must redefine the entire table in the unpacker options.
